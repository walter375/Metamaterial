...
class Beam:
    def __init__(self, c_p, i_p, j_p,
        ...
        self.border = border
        self.borderWithoutPosDisplaced = borderWithoutPosDisplaced
        self.x = x
        self.y = y
    ...
    def displacementSensitivityObjective(self, cGuess_p, ric_flat, beamlengths_p, r_orig_ic,
        ...
        """ dim: dimension of optimizePos1, 0: x, 1: y, 2: xy """
        self.c_p = cGuess_p
        #print("sensitive:\n",c_p)
        ...
        if dim == 2:
            dfdr[optimizePos1] = 2 * (r_current_ic[optimizePos1]
                                       - self.r_orig_ic[optimizePos1])
        else:
            dfdr[optimizePos1, dim] = 2 * (r_current_ic[optimizePos1, dim]
                                       - self.r_orig_ic[optimizePos1, dim])
        # print("gradient:\n", gradient)
        ...
        if self.dimDisplaced == 0:
            dfdr[self.posDisplaced, 0] = -(r_current_ic[self.posDisplaced, 0] - r_orig_ic[self.posDisplaced, 0])
        elif self.dimDisplaced == 1:
            dfdr[self.posDisplaced, 1] = -(r_current_ic[self.posDisplaced, 1] - r_orig_ic[self.posDisplaced, 1])
        elif self.dimDisplaced == 2:
            dfdr[self.posDisplaced] = -(r_current_ic[self.posDisplaced] - r_orig_ic[self.posDisplaced])
...
#todo test
def getBorderPoints(r_ic, posDisplaced, left=False, right=False, lower=False, upper=False):
    ...
    borderWithoutPosDisplaced = np.sort(border)
    border = np.sort(np.append(border, posDisplaced))
    return border, borderWithoutPosDisplaced
...
def constrainHessian(hessian, border, posDisplaced, dimDisplaced):
    ...
    return hessian
def runOptimizer(function, x0, arguments, cons={}, gradient=None, hessian=None, method='l-bfgs-b'):
    res = scipy.optimize.minimize(function, x0=x0,  args=(arguments), constraints=cons, jac=gradient, hess=hessian, method=method)
    return res.x
def plotResults(points1, points2, points3, points4, c1, c2, c3, c4):
    ...
    points4 = ricUnflat(points4, r_stressed_ic, border, x, y)
    f, ([ax1, ax2], [ax3, ax4]) = plt.subplots(2, 2, sharex=all, sharey=all, figsize=(7, 7))
    ...
    for i, j in zip(i_p, j_p):
        ...
        ax1.plot([points1[i][0], points1[j][0]], [points1[i][1], points1[j][1]],
                 linewidth=c1[i]*2, marker= 'o', markerfacecolor='k',
                 markeredgewidth=marker_edge_width, markeredgecolor='k',
                 linestyle='-', color='k')
        ax2.plot([points2[i][0], points2[j][0]], [points2[i][1], points2[j][1]],
                 linewidth=c2[i]*2, marker= 'v', markerfacecolor='r',
                 markeredgewidth=marker_edge_width, markeredgecolor='k',
                 linestyle='-', color='r')
        ax3.plot([points3[i][0], points3[j][0]], [points3[i][1], points3[j][1]],
                 linewidth=c3[i]*2, marker='D', markerfacecolor='g',
                 markeredgewidth=marker_edge_width, markeredgecolor='k',
                 linestyle='-', color='g')
        ax4.plot([points4[i][0], points4[j][0]], [points4[i][1], points4[j][1]],
                 linewidth=c4[i]*2, marker= 's', markerfacecolor='purple',
                 markeredgewidth=marker_edge_width, markeredgecolor='k',
                 linestyle='-', color='purple')
    for i, j in zip(i_p, j_p):
        ax2.plot([r_orig_ic[i][0], r_orig_ic[j][0]], [r_orig_ic[i][1], r_orig_ic[j][1]], 'xb:', linewidth=2)
        ax1.plot([r_orig_ic[i][0], r_orig_ic[j][0]], [r_orig_ic[i][1], r_orig_ic[j][1]], 'xb:', linewidth=2)
        ax3.plot([r_orig_ic[i][0], r_orig_ic[j][0]], [r_orig_ic[i][1], r_orig_ic[j][1]], 'xb:', linewidth=2)
        ax4.plot([r_orig_ic[i][0], r_orig_ic[j][0]], [r_orig_ic[i][1], r_orig_ic[j][1]], 'xb:', linewidth=2)

    # f.suptitle("Optimization of position %i" % (optPos))
    ax1.set_title(r'(a) Beam model') # "Angle+Beam, s_b=%i, s_a=%i" %(stiffness_beam, stiffness_angle))
    ax2.set_title(r'(b) Angle model')
    ax3.set_title(r'(c) Beam+Angle, $s_b$=%i, $s_a$=%i' % (stiffness_beam1,stiffness_angle1))
    ax4.set_title(r'(d) Beam+Angle, $s_b$=%i, $s_a$=%i' % (stiffness_beam2, stiffness_angle2))

    # ax2.set_title("(b) Minimized x movement")
    # ax3.set_title("(c) Minimized y movement")
    # ax4.set_title("(d) Minimized xy movement")

    for ax in [ax1, ax2, ax3, ax4]:
        ax.grid(True)
        # ax.set_aspect('equal')  # Ensure the x and y axes are scaled the same
        ax.set(xlabel='x / beamlength', ylabel='y / beamlength')
...
if __name__ == "__main__":
    ...
    stiffness_bounds = scipy.optimize.Bounds(lb=1, ub=50, keep_feasible=True)
    ...
    left = True
    right = False
    border, borderWithoutPosDisplaced = getBorderPoints(r_stressed_ic,posDisplaced, left, right)
    ''' beams '''
    beamlengths_p = getBeamLength(r_orig_ic, i_p, j_p)
    c_p = np.ones(nb_bodies)
    beam = Beam(c_p, i_p, j_p,
                r_orig_ic, r_stressed_ic,
                border, borderWithoutPosDisplaced,
                x, y,
                posDisplaced, dimDisplaced)
    ...
    p2 = runOptimizer(angle.UAngleObjective, ric_flat, cos0_t, cons=cons, gradient=angle.gradientUAngleObjective,  method='SLSQP')
    ...
    plotResults(p1, p2, p3, p4, c_p,c_p,c_p,c_p)
    ...
    # right = 0
    #
    # border, borderWithoutPosDisplaced = getBorderPoints(r_orig_ic, posDisplaced, left, right  )
    #
    # ''' beams '''
    # c_p = np.ones(nb_bodies)
    ...
    # # angle.c_t = np.full(nb_angles, stiffness_angle1)
    # # beam.c_p = np.full(nb_bodies, stiffness_beam1)
    # p3 = runOptimizer(angle.UAngleObjective, ric_flat, cos0_t, cons=cons, gradient=angle.gradientUAngleObjective,  method='SLSQP')
    # stiffness_angle2 = 1
    # stiffness_beam2 = 40
    # angle.c_t = np.full(nb_angles, stiffness_angle2)
    # beam.c_p = np.full(nb_bodies, stiffness_beam2)
    ...
    from Structures import gripperWithHinges as g1
    r_orig_ic = g1.r_orig_ic
    posDisplaced = g1.posDisplaced
    dimDisplaced = g1.dimDisplaced
    distanceDisplaced = g1.distanceDisplaced
    r_stressed_ic = g1.r_stressed_ic
    i_p = g1.i_p
    j_p = g1.j_p
    i_t = g1.i_t
    j_t = g1.j_t
    ...
    left = 1
    right = 0
    border, borderWithoutPosDisplaced = getBorderPoints(r_stressed_ic, posDisplaced,1,0,0,0)
    # stiffness_beam = 50
    ''' beams '''
    beamlengths_p = getBeamLength(r_orig_ic, i_p, j_p)
    c_p = np.ones(nb_bodies)
    ...
    from Structures import Auxetic as a
    r_orig_ic = a.r_orig_ic
    r_stressed_ic = a.r_stressed_ic
    posDisplaced = a.posDisplaced
    dimDisplaced = a.dimDisplaced
    distanceDisplaced = a.distanceDisplaced
    i_p = a.i_p
    j_p = a.j_p
    i_t = a.i_t
    j_t = a.j_t
    ...
    from Structures import gripperWithoutHinges as g2
    # r_orig_ic = g2.r_orig_ic
    # r_stressed_ic = g2.r_stressed_ic
    # posDisplaced = g2.posDisplaced
    # dimDisplaced = g2.dimDisplaced
    # distanceDisplaced = g2.distanceDisplaced
    # i_p = g2.i_p
    # j_p = g2.j_p
    # i_t = g2.i_t
    # j_t = g2.j_t
    ...
    """ Optimization"""
    from Structures import testcase as t
    # optPos = 1
    # stiffness_bounds = scipy.optimize.Bounds(lb=1, ub=50, keep_feasible=True)
    # r_orig_ic = t.r_orig_ic
    # r_stressed_ic = t.r_stressed_ic
    # posDisplaced = t.posDisplaced
    # dimDisplaced = t.dimDisplaced
    # distanceDisplaced = t.distanceDisplaced
    # i_p = t.i_p
    # j_p = t.j_p
    # nb_bodies = i_p.shape[0]
    # nb_positions = r_orig_ic.shape[0]
    ...
    # right = 0
    #
    # border, borderWithoutPosDisplaced = getBorderPoints(r_orig_ic, posDisplaced,
    #                                                     0, 0, 1, 0)
    # beamlengths_p = getBeamLength(r_orig_ic, i_p, j_p)
    # c_p = np.ones(nb_bodies)
    # beam = Beam(c_p, i_p, j_p,
    ...
    #                   cons={}, gradient=beam.gradientUBeamObjective)
    # dim = 0
    # c0_p = np.random.rand(nb_bodies) * 10
    ...
    # cOptimize4 = scipy.optimize.minimize(beam.displacementObjective,
    #                                      x0=c_p,
    #                                      args=(ric_flat, beamlengths_p, r_orig_ic, optPos, dim),
    #                                      jac=beam.displacementSensitivityObjective,
    #                                      bounds=stiffness_bounds,
    ...
    #                                               'maxiter': 1000})
    # # p3 = np.zeros_like(p2)
    # beam.c_p = cOptimize4.x
    # p4 = runOptimizer(beam.UBeamObjective, ric_flat, beamlengths_p, cons={}, gradient=beam.gradientUBeamObjective)
    #
    # plotResults(p1, p2, p3, p4, c_p, cOptimize2.x, cOptimize3.x, cOptimize4.x)
    # plotResults(p1, p2, p3, p4, c_p*4, c_p*4, c_p*4, c_p*4)
    # r_current = ricUnflat(p2, r_stressed_ic, border, x, y)
    # print("c0", c_p)
    # print("cOptimize", cOptimize2.x)
    # print("cOptimize3", cOptimize3.x)
    # print("cOptimize4", cOptimize4.x)
    ...
    # dim = 1
    # # rOptimized_flat = ricFlat(p1, border, x, y)
    # stiffness_bounds = scipy.optimize.Bounds(lb=0.1, ub=100, keep_feasible=True)
    # cOptimize2 = scipy.optimize.minimize(beam.displacementObjective,
    #                                     x0=c0_p,
    #                                     args=(ric_flat, beamlengths_p, r_orig_ic, optPos, dim),
    #                                     #jac=beam.displacementSensitivityObjective,
    ...
    # p4 = runOptimizer(beam.UBeamObjective, ric_flat, beamlengths_p, cons={}, gradient=beam.gradientUBeamObjective)
    #
    # #plotResults(p1, p2, p3, p4, np.ones_like(c_p)*10, np.ones_like(c_p)*10, np.ones_like(c_p)*10, np.ones_like(c_p)*10)
    ...
    #
    # plotResults(p1, p2, p3, p4, np.ones_like(c_p)*10,np.ones_like(c_p)*10,np.ones_like(c_p)*10,np.ones_like(c_p)*10)
    # plotResults(p1, p2, p3, p4, np.ones_like(c_p)*10, cOptimize1.x, cOptimize2.x, cOptimize3.x)
    #
    # r_current = ricUnflat(p2, r_stressed_ic, border, x, y)
    # print(r_current[optPos] - r_orig_ic[optPos])
    # print("cOptimize1", cOptimize1.x)
    ...
    #
    # plotResults(p1, p2, p3, p4, np.ones_like(c_p)*10, np.ones_like(c_p)*10, np.ones_like(c_p)*10,np.ones_like(c_p)*10)
    # plotResults(p1, p2, p3, p4, cOptimize1.x, cOptimize2.x, cOptimize3.x,beam.c_p)
    # r_current = ricUnflat(p2, r_stressed_ic, border, x, y)
    # print(r_current[optPos] - r_orig_ic[optPos])
    ...
    """ Inverter Mechanism """
    from Structures import InverterMechanism_improved as im
    # r_orig_ic = im.r_orig_ic
    # r_stressed_ic = im.r_stressed_ic
    # posDisplaced = im.posDisplaced
    # dimDisplaced = im.dimDisplaced
    ...
    #
    # ''' beams '''
    # c_p =  np.ones(nb_bodies)*3 # np.random.rand(nb_bodies)*10 #
    # beam = Beam(c_p, i_p, j_p, r_orig_ic, r_stressed_ic,
    #             border, borderWithoutPosDisplaced, x, y,
    #             posDisplaced, dimDisplaced)
    # beamlengths_p = getBeamLength(r_orig_ic, i_p, j_p)
    ...
    #
    # p1 = runOptimizer(beam.UBeamObjective, ric_flat, beamlengths_p, cons={}, gradient=beam.gradientUBeamObjective)
    # c0_p = np.random.rand(nb_bodies) * 20 #c_p#np.full_like(c_p, 1) #
    # print(c0_p)
    # dim = 0
    # # rOptimized_flat = ricFlat(p1, border, x, y)
    # stiffness_bounds = scipy.optimize.Bounds(lb=1, ub=100, keep_feasible=True)
    # cOptimize1 = scipy.optimize.minimize(beam.displacementObjective,
    #                                     x0=c0_p,
    #                                     args=(ric_flat, beamlengths_p, r_orig_ic, optPos, dim, optPos2),
    #                                     jac=beam.displacementSensitivityObjective,
    #                                     bounds=stiffness_bounds,
    #                                     method='l-bfgs-b',
    #                                     options={'gtol': 1e-10,
    ...
    # plotResults(p1, p2, p3, p4, c0_p, cOptimize1.x, cOptimize2.x, cOptimize3.x)
    # #plotResults(p1, p2, p3, p4, np.ones_like(c_p)*10,np.ones_like(c_p)*10,np.ones_like(c_p)*10,np.ones_like(c_p)*10)
    # r_current = ricUnflat(p1, r_stressed_ic, border, x, y)
    # print(r_current[optPos] - r_current[optPos2])
    # r_current = ricUnflat(p2, r_stressed_ic, border, x, y)
    # print(r_current[optPos] - r_current[optPos2])
    # r_current = ricUnflat(p3, r_stressed_ic, border, x, y)
    # print(r_current[optPos] - r_current[optPos2])
    # r_current = ricUnflat(p4, r_stressed_ic, border, x, y)
    # print(r_current[optPos] - r_current[optPos2])

    """ Gripper """
    from Structures import gripperWithHinges_improved as g1
    # r_orig_ic = g1.r_orig_ic
    # posDisplaced = g1.posDisplaced
    # dimDisplaced = g1.dimDisplaced
    # distanceDisplaced = g1.distanceDisplaced
    ...
    #
    # p1 = runOptimizer(beam.UBeamObjective, ric_flat, beamlengths_p, cons={}, gradient=beam.gradientUBeamObjective)
    # c0_p = np.random.rand(nb_bodies) * 50  # np.full_like(c_p, 15)
    # print(c0_p)
    # dim = 0
    # # rOptimized_flat = ricFlat(p1, border, x, y)
    ...
    #
    # plotResults(p1, p2, p3, p4, c0_p, cOptimize1.x, cOptimize2.x, cOptimize3.x)
    # plotResults(p1, p2, p3, p4, c_p*10, c_p*10, c_p*10, c_p*10)
    # print("cOptimize1", cOptimize1.x)
    # print("cOptimize2", cOptimize2.x)
    # print("cOptimize3", cOptimize3.x)
    # r_current = ricUnflat(p1, r_stressed_ic, border, x, y)
    # print(r_current)# - r_current[optPos2])
    # r_current = ricUnflat(p2, r_stressed_ic, border, x, y)
    # print(r_current[optPos] )#- r_current[optPos2])
    # r_current = ricUnflat(p3, r_stressed_ic, border, x, y)
    # print(r_current[optPos] )#- r_current[optPos2])
    ...
    # beamlengths_p = getBeamLength(r_orig_ic, i_p, j_p)
    # c_p = np.ones(nb_bodies)
    # beam = Beam(c_p, i_p, j_p, r_orig_ic, r_stressed_ic, border, borderWithoutPosDisplaced, x, y, posDisplaced,
    #             dimDisplaced)
    #
    # ric_flat = ricFlat(r_stressed_ic, border, x, y)
    # p1 = runOptimizer(beam.UBeamObjective, ric_flat, beamlengths_p, cons={}, gradient=beam.gradientUBeamObjective)
    ...
    # # p3 = np.zeros_like(p2)
    # p4 = np.zeros_like(p2)
    # plotResults(p1, p2, p3, p4, np.ones_like(c_p), np.ones_like(c_p), np.ones_like(c_p), np.ones_like(c_p))
    # r_current = ricUnflat(p2, r_stressed_ic, border, x, y)
    # print(r_current[optPos] - r_orig_ic[optPos])
